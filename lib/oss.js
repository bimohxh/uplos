...
const oss = {
    ...
    async checkFiles(action) {
        ...
        // 处理 remote 路径：./ 或 . 转换为空字符串（OSS 根目录）
        const remotePrefix = oss.normalizeRemotePath(action.remote);
        const remoteFiles = await aliyun.getRemoteFiles(remotePrefix);
    ...
    async uploadFile(action, file) {
        const customHeaders = oss.getMetaHeaders(action, file.remote);
        await action.aliyun.upload(path.resolve(process.cwd(), file.local), file.remote, customHeaders);
    },

    // 获取文件的 meta headers
    getMetaHeaders(action, remoteFile) {
        ...
        for (const [pattern, headers] of Object.entries(action.meta)) {
            const ig = ignore().add(pattern);
            if (ig.ignores(remoteFile)) {
                matchedHeaders = { ...matchedHeaders, ...headers };
            }
        }
    ...
    normalizeRemotePath(remotePath) {
        if (!remotePath || remotePath === '.' || remotePath === './') {
            return ''; // OSS 根目录用空字符串表示
        }
        // 移除开头的 ./
        let normalized = remotePath.replace(/^\.\//, '');
        // 确保以 / 结尾（如果不为空）
