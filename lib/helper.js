...
let Helper = {
            ...
            "actions": [
                {
                    ...
                    "remove_prefix": "",
                    "ignore": ["*.log", "*.log.json", "*.config", "*.config.json", ".gitignore", "README.md", ".git", "node_modules", "package.json", "package-lock.json"],
                    "whitelist": [],
                }
            ] 
        };
  ...
  // 浏览文件夹下的所有文件
  walk: (dir, isIgnore, hasWhiteList) => {
    let results = []
    let list = fs.readdirSync(dir);
    for (const file of list) {
        const fullFile = path.resolve(dir, file);
        const relativePath = path.relative(process.cwd(), fullFile);
        const shoudIgnore = isIgnore(relativePath);
        if (shoudIgnore && !hasWhiteList) { // 开启白名单则是需要全部遍历的
            continue;
        }
        var stat = fs.statSync(fullFile)
        if (stat && stat.isDirectory()) {
            results = results.concat(Helper.walk(fullFile, isIgnore))
        } else {
          !shoudIgnore && results.push(relativePath);
        }
    }
